...
async function main() {
    console.log("Seeding Pokémon species…");
    for (const p of POKEMON) {
        await prisma.pokemon.upsert({
            where: { number: p.number },
            update: { name: p.name, types: p.types },
            create: { number: p.number, name: p.name, types: p.types },
        });
    }
    console.log(`Done — ${POKEMON.length} Pokémon seeded.`);
}
